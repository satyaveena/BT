...
Public Class RankCart
    ...
    Public Event CallBackRankAttemptComplete As EventHandler(Of CartAttemptCompleteEventArgs)
    Public Event CallBackExLogIt As EventHandler(Of ExLogItEventArgs)

    Dim WithEvents StartTimer As New Timers.Timer
    Dim cart2Rank As RankRequestsRankRequest
    ...
    Public Sub New(ByVal Cart As RankRequestsRankRequest, ByVal attempts As Short, ByVal delay As Short, ByVal xmlFileName As String, ByVal utblESPStatus As DataTable, ByVal utblBranches As DataTable, ByVal utblFundCodes As DataTable)
        cart2Rank = Cart
        attemptNo = attempts
        delaySecs = delay
        xmlFile = xmlFileName
    ...
    Private Sub StartTimer_Elapsed(sender As Object, e As Timers.ElapsedEventArgs) Handles StartTimer.Elapsed
        ...
        RankACart(cart2Rank, attemptNo, delaySecs, xmlFile)

    End Sub

    Public Sub RankACart(ByVal cart As RankRequestsRankRequest, ByVal attempts As Short, ByVal delaysecs As Short, ByVal xmlFileName As String)
        ...
        'we're a go. first create the serializer to work on the Class created to hold RankRequests for serializing
        Dim jsonSerializer As DataContractJsonSerializer
        jsonSerializer = New DataContractJsonSerializer(GetType(ESPWinServices.RankRequestJson))
        ...
        Dim rankResponse As ESPWinServices.RankResponseJson = Nothing
        Dim jsonCart As ESPWinServices.RankRequestJson
        Dim uri As String
        ...
        Try
            ...
            uri = My.Settings.ESPBaseURI & "jobs/rank"

            'but for unit testing we overwrite with my local RESTService
            'uri = "http://localhost:63062/RESTService/esp/jobs/rank"
            ...
            If InStr(resp.ContentType.ToUpper, "JSON") > 0 Then
                ...
                Try
                    ...
                    FileItUTF8(My.Settings.LogDir, "json", "RankResp_" & cart.CartID, jsonString)
                    l = 4
                    'then create the serializer to read json into the class
                    jsonSerializerR = New DataContractJsonSerializer(GetType(ESPWinServices.RankResponseJson))
                    l = 5
                    ...
                    rankResponse = CType(jsonSerializerR.ReadObject(memstrJson), ESPWinServices.RankResponseJson)
                    l = 7
                    'which leaves you the json string and the class created from it both accessible as needed.
                    l = 8
                    Logit("Rank Json Status:" & rankResponse.statusCode & " rank json Description:" & rankResponse.statusMessage)

                Catch ex As Exception

                    Logit(l & "Exception Encountered trying to get json from ranking job HTTP response: " & ex.Message)
                    If Not (ex.StackTrace Is Nothing) Then Logit(ex.StackTrace)
                    ExLogIt(uri, "Exception Encountered trying to get json from ranking job HTTP response: " & ex.Message)
            ...
            If My.Settings.DebugLogging = True Then Logit("Rank HTTP Response: " & resp.StatusCode & "-" & resp.StatusDescription)

            If Not IsNothing(rankResponse) Then
                Logit("RankStatusCode=" & rankResponse.statusCode.ToString & ", RankStatusMessage=" & rankResponse.statusMessage & ", Job ID =" & rankResponse.jobId)
                ESPStateCartStatus = rankResponse.statusMessage
            Else
                Logit("Rank JSON Content not found")
            ...
            If resp.StatusCode = HttpStatusCode.Accepted Or resp.StatusCode = HttpStatusCode.OK Then
                Logit("All is well for ranking Cart=" & cart.BasketName & " So I will update it's status " & "for CartID=" & cart.CartID)

                'populate the user table for ESPState with response and/or original data
                utblESP.Rows.Add(cart.CartID, "RANK", "Submitted", rankResponse.jobId, ESPStateCartStatus, "No Errors", jsonString)
                ...
                Catch upex As Exception
                    Logit("RANK: Couldn't update ESPState w/Success: " & upex.Message)
                End Try
                If Not cretvalue = 0 Then
                    'Send an Alert that the SP returned an error.
                    Dim AlertMess As String = "Error Calling SP procESPSetESPState for ranking CartID=" & cart.CartID & " Return value=" & cretvalue.ToString & " Message=" & cerrMess
                ...
                Else
                    If My.Settings.DebugLogging = True Then Logit("Results of procESPSetESPState for ranking CartID=" & cart.CartID & " Return value=" & cretvalue.ToString & " Message=" & cerrMess)
                End If

                'Note, we don't re-attempt the Cart Rank at this point, even if the call to update it's status failed, 'cause we think the rank submission succeeded.
            ...
            Else
                ...
                If Not lastAttempt Then
                    ...
                    Logit("I got an HTTP Status of " & resp.StatusCode.ToString & " for ranking Cart=" & cart.BasketName & " so I think I failed because it's not 'OK' or 'ACCEPTED'. Will Retry. This was attempt " & attemptNo.ToString & " of " & (Val(My.Settings.CartJobsRetryCount) + 1).ToString)

                Else

                    Dim mess As String = "I got an HTTP Status of " & resp.StatusCode.ToString & " for ranking Cart=" & cart.BasketName & " so I think I failed because it's not 'OK' or 'ACCEPTED'. Retries Exhausted. User Alert goes to " & cart.UserGuid

                    Dim exMess As String = "I got an HTTP Status of " & resp.StatusCode.ToString & " for ranking Cart=" & cart.BasketName

                    If Not IsNothing(rankResponse) Then
                        mess = mess & " RankStatusCode=" & rankResponse.statusCode.ToString & ", RankStatusMessage=" & rankResponse.statusMessage & ", Job ID =" & rankResponse.jobId
                        exMess = exMess & " RankStatusCode=" & rankResponse.statusCode.ToString & ", RankStatusMessage=" & rankResponse.statusMessage & ", Job ID =" & rankResponse.jobId
                    End If
                    Logit(mess)
                    ExLogIt("RankCart", exMess)

                    Dim UACall As Boolean = True
                    Dim UATempl As Integer = UserAlertsService.AlertMessageTemplateIDEnum.ESPRankFailSystem
                    If resp.StatusCode = HttpStatusCode.BadRequest Then UATempl = UserAlertsService.AlertMessageTemplateIDEnum.ESPRankFailLibrary

                    UACall = SendUserAlert(cart, UATempl)

                    If Not UACall Then
                        Logit("User Alert Call Failed for user=" & cart.UserGuid & " and ranking Basket=" & cart.CartID)
                        ExLogIt("UserAlertsService", "User Alert Call Failed for user=" & cart.UserGuid & " and ranking Basket=" & cart.CartID)
                    End If

                    utblESP.Rows.Add(cart.CartID, "RANK", "Failed", DBNull.Value, ESPStateCartStatus, mess, jsonString)
                    ...
                    Catch upex As Exception
                        Logit("RANK: Couldn't update ESPState w/Fail: " & upex.Message)
                    End Try
                    If Not cretvalue = 0 Then
                        'Send an Alert that the SP returned an error.
                        Dim AlertMess As String = "Error Calling SP procESPSetESPState for ranking CartID=" & cart.CartID & " Return value=" & cretvalue.ToString & " Message=" & cerrMess
        ...
        Catch wex As WebException
            ...
            If Not lastAttempt Then
                Logit("I got a Web Exception of " & wex.Message & " for ranking Cart=" & cart.BasketName & " so The Ranking failed. Will retry. This was attempt " & attemptNo.ToString & " of " & Val(My.Settings.CartJobsRetryCount + 1).ToString)


            Else

                Dim mess As String = "I got a Web Exception of " & wex.Message & " for ranking Cart=" & cart.BasketName & " so The Ranking failed, retrys exhausted, & UserAlert goes to " & cart.UserGuid
                Dim jmess As String = ""
                ...
                If InStr(httpResponse.ContentType.ToUpper, "JSON") > 0 Then
                    ...
                    Try
                        ...
                        If Not IsNothing(rankResponse) Then
                            jmess = mess & vbCrLf & " Ranking Json Status:" & rankResponse.statusCode & " json Description:" & rankResponse.statusMessage & ", Job ID =" & rankResponse.jobId
                            Logit(jmess)
                            ESPStateCartStatus = rankResponse.statusMessage
                        Else
                    ...
                    Catch ex As Exception
                        ...
                        Logit("Exception Encountered trying to get json from jobs/rank HTTP response: " & ex.Message)
                        ExLogIt(My.Settings.ESPBaseURI & "jobs/rank", "Exception Encountered trying to get json from HTTP response: " & ex.Message)
                        ESPStateCartStatus = httpResponse.StatusDescription
                ...
                Else
                    With httpResponse
                        jmess = mess & vbCrLf & "No JSON found in ranking httpResponse. Content.Type=" & httpResponse.ContentType & " Content.Length=" & httpResponse.ContentLength.ToString
                        Logit(jmess)
                    End With
                End If
                ...
                Select Case exStatusCode
                    Case Is = HttpStatusCode.NotFound
                        UATempl = UserAlertsService.AlertMessageTemplateIDEnum.ESPRankFailLibrary

                    Case Else
                        UATempl = UserAlertsService.AlertMessageTemplateIDEnum.ESPRankFailSystem
                ...
                Catch upex As Exception
                    Logit("RANK: Couldn't update ESPState w/WebEx: " & upex.Message)
                End Try
                If Not cretvalue = 0 Then
                    'Send an Alert that the SP returned an error.
                    Dim AlertMess As String = "Error Calling SP procESPSetESPState for ranking CartID=" & cart.CartID & " Return value=" & cretvalue.ToString & " Message=" & cerrMess
        ...
        Catch ex As Exception
            ...
            If lastAttempt Then
                Logit("I got a General Exception of " & ex.Message & " for ranking Cart=" & cart.BasketName & " so The Ranking failed. Will Retry. This was attempt " & attemptNo.ToString & " of " & Val(My.Settings.CartJobsRetryCount + 1).ToString)

            Else

                Dim mess As String = "I got a General Exception of " & ex.Message & " for ranking Cart=" & cart.BasketName & " so The Ranking failed, Retries Exhausted, & UserAlert goes to: " & cart.UserGuid
                Logit(mess)
                ExLogIt("RankCart", mess)

                Dim UACall As Boolean = True
                Dim UATempl As UserAlertsService.AlertMessageTemplateIDEnum = UserAlertsService.AlertMessageTemplateIDEnum.ESPRankFailSystem
                ...
                utblESP.Rows.Add(cart.CartID, "RANK", "Failed", DBNull.Value, "System Exception", mess, jsonString)
                Try
                    ctbladapter.procESPSetESPState(utblESP, utblBRA, utblFUN, "OK")
                    cretvalue = ctbladapter.GetReturnValue(5)
                    cerrMess = ctbladapter.GetParam4(5)
                ...
                Catch upex As Exception
                    Logit("RANK: Couldn't update ESPState w/GenEx: " & upex.Message)
                End Try
        ...
        RaiseEvent CallBackRankAttemptComplete(Me, rc)
    ...
    Function SendUserAlert(ByVal cart As RankRequestsRankRequest, ByVal template As UserAlertsService.AlertMessageTemplateIDEnum) As Boolean
