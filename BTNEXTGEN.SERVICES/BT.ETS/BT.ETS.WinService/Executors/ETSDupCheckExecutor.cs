...
namespace BT.ETS.WinService.Executors
{
    public class ETSDupCheckExecutor : BaseETSExecutor, IQueueItemExecutor
    {
        ...
        public override string ETSPostBackServiceUrl { get { return AppSettings.EspReceiveDupCheckUrl; } }

        public async Task ExecuteRequest(ETSQueueItem etsQueueItem)
        {
            TextFileLogger.LogInfo("DupCheck Executing. JobID: " + etsQueueItem.JobID);

            try
            {
                // use to sent to EST service
                var etsResultPostBack = new EtsServiceQueueBackgroundResult<DupCheckResult>();
                ...
                try
                {
                    ...
                    dupCheckResult = await ETSRequestHandler.GetDupChecks(etsQueueItem.DupCheckRequest);
                    etsResultPostBack.Data = dupCheckResult;
                    if (dupCheckResult.ErrorItems != null && dupCheckResult.ErrorItems.Count > 0)
                    {
                ...
                {
                    TextFileLogger.LogInfo(string.Format("DupCheck BusinessException - GetDupChecks: {0}. StackTrace: {1}", ex.Message, ex.StackTrace));

                    etsResultPostBack.StatusCode = ex.ErrorCode;
                    etsResultPostBack.StatusMessage = ex.Message;
                    etsResultPostBack.Data = new DupCheckResult();//TFS36707
                }
                catch (Exception ex)
                {
                    isError = true;
                    TextFileLogger.LogException(string.Format("DupCheck Exception JobID: {0} - GetDupChecks: {1}", etsQueueItem.JobID, ex.Message), ex);
                ...
                TextFileLogger.LogInfo("DupCheck got Result successfully. JobID: " + etsQueueItem.JobID);

                // Update ETS Queue with DupCheck Result
                await CommonDAO.Instance.UpdateETSQueueItem(etsQueueItem.JobID, this.MongoResponseFieldName, dupCheckResult);
            ...
            {
                TextFileLogger.LogException(string.Format("DupCheck ExecuteRequest Exception JobID: {0}", etsQueueItem.JobID), ex);
                throw;
            }

            TextFileLogger.LogInfo("DupCheck Finished. JobID: " + etsQueueItem.JobID);
        }
    }
}
