...
namespace BT.TS360API.SearchService.Services
{
    public partial class SearchService
    {
        ...
        public AppServiceResult<List<ArtifactItems>> GetArtifactsByWork(string workId)
        {
            var ajaxResult = new AppServiceResult<List<ArtifactItems>>();
            try
            {
                var artifactsClientId = GlobalConfiguration.ReadAppSetting(GlobalConfigurationKey.ArtifactsClientId);
                var artifactsClientSecret = GlobalConfiguration.ReadAppSetting(GlobalConfigurationKey.ArtifactsClientSecret);
                var artifactsApiUrl = GlobalConfiguration.ReadAppSetting(GlobalConfigurationKey.ArtifactsApiUrl);

                var artifacts = ArtifactsHelper.GetArtifactsByWork(workId, artifactsClientId, artifactsClientSecret, artifactsApiUrl);
                var artifactTypesToRemove = GetArtifactTypesToRemove();
                ajaxResult.Data = RemoveArtifactsByType(artifacts, artifactTypesToRemove);
            ...
            catch (Exception ex)
            {
                ...
                ajaxResult.Data = new List<ArtifactItems>();
            }
            return ajaxResult;
        }

        private List<ArtifactItems> RemoveArtifactsByType(List<ArtifactItems> artifacts, List<ArtifactBrowsingItem> artifactTypesToRemove)
        {
            var result = new List<ArtifactItems>();
            foreach (var artifactItem in artifacts)
            {
                var artifactTypeUCID = "";
                var removeItem = false;
                foreach (var artifact in artifactItem.Data)
                {
                    if (string.Equals(artifact.name, "artifact_type_id"))
                    {
                        artifactTypeUCID = artifact.value;
                        break;
                    }
                }
                if (!string.IsNullOrEmpty(artifactTypeUCID))
                {
                    foreach (var artifactBrowsingItem in artifactTypesToRemove)
                    {
                        if (string.Equals(artifactTypeUCID, artifactBrowsingItem.ArtifactTypeUCID))
                        {
                            removeItem = true;
                            break;
                        }
        ...
        private List<ArtifactBrowsingItem> GetArtifactTypesToRemove()
        {
            var siteTermCacheKey = "ArtifactsToRemove";
            var result = CachingController.Instance.Read(siteTermCacheKey) as List<ArtifactBrowsingItem>;
            if (result != null) return result;

            var categoryNameList = new List<string> { "Readability", "Awards" };
            result = ProductCatalogDAO.Instance.GetArtifactTypesByCategoryName(categoryNameList);

            CachingController.Instance.Write(siteTermCacheKey, result, 360);
