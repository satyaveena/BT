    ...
    internal class OAuth2AuthorizationServer : IAuthorizationServerHost
    {
        ...
        public AccessTokenResult CreateAccessToken(IAccessTokenRequest accessTokenRequestMessage)
        {
            try
            {
                ...
                var expiration = ConfigurationManager.AppSettings[Constants.ACCESS_TOKEN_EXPIRATION];
                double minute;
                if (!Double.TryParse(expiration, out minute)) minute = 10;//default
                ...
                accessToken.Lifetime = TimeSpan.FromMinutes(minute);
                ...
                //string message = string.Format("SSO ACCESS TOKEN REQUEST MESSAGE- ClientId: {0} - UserName: {1}",
                //    accessTokenRequestMessage.ClientIdentifier, accessTokenRequestMessage.UserName);

                //Logger.Write("PROFILES_OAUTH_SSO", message, false);

                //HttpContext.Current.Session["SSO_CLIENT_ID"] = accessTokenRequestMessage.ClientIdentifier;
                //HttpContext.Current.Session["SSO_USER_NAME"] = accessTokenRequestMessage.UserName;
        ...
        {
            //var consumerRow = Utilities.DataContext.Clients.SingleOrDefault(
            ...
            //return consumerRow;
            IClientDescription client = GetClientObject(clientIdentifier);

            return client;
        }

        private Client GetClientObject(string clientIdentifier)
        {
            try
            {
                var oAuthDao = new OAuthDAO();
                var ds = oAuthDao.GetClient(clientIdentifier);
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    var row = ds.Tables[0].Rows[0];
                    var client = new Client()
                    {
                        ClientId = DatabaseHelper.ConvertToInt(row["ClientId"]),
                        ClientIdentifier = DatabaseHelper.ConvertToString(row["ClientIdentifier"]),
                        Callback = DatabaseHelper.ConvertToString(row["Callback"]),
                        ClientSecret = DatabaseHelper.ConvertToString(row["ClientSecret"]),
                        Name = DatabaseHelper.ConvertToString(row["Name"]),
                        ClientType = DatabaseHelper.ConvertToInt(row["ClientType"])
                    };
                    return client;
                }
            }
        ...
        public bool CanBeAutoApproved(EndUserAuthorizationRequest authorizationRequest, string loginName)
        {
            ...
            if (authorizationRequest.ResponseType == EndUserAuthorizationResponseType.AuthorizationCode)
            {
                ...
                var requestingClient = this.GetClientObject(authorizationRequest.ClientIdentifier);//Utilities.DataContext.Clients.First(c => c.ClientIdentifier == authorizationRequest.ClientIdentifier);
        ...
        private bool IsAuthorizationValid(HashSet<string> requestedScopes, string clientIdentifier, 
            DateTime issuedUtc, string username)
        {
            try
            {
                ...
                //var grantedScopeStrings = from auth in Utilities.DataContext.ClientAuthorizations
                //                          where
                ...
                //                              auth.User.OpenIDFriendlyIdentifier == username
                //                          select auth.Scope;
                var oAuthDao = new OAuthDAO();
                var grantedScopeStrings = oAuthDao.GetScopes(clientIdentifier, issuedUtc, username);
