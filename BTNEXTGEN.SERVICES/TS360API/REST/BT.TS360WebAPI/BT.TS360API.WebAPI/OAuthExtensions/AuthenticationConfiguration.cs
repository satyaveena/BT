...
namespace BT.TS360API.WebAPI.OAuth {
    ...
    public class AuthenticationHandler : DelegatingHandler {
        private readonly string HeaderBearer = "Bearer ";
                ...
                if (string.CompareOrdinal(authHeader, 0, header, 0, header.Length) == 0) {
                    using (var signing = _configuration.CreateAuthorizationServerSigningServiceProvider()) {
                        using (var encrypting = _configuration.CreateResourceServerEncryptionServiceProvider()) {
                            ...
                            var scope = ExtractScopeFromRequest(request.RequestUri.AbsoluteUri);
                            var principal = resourceServer.GetPrincipal(request, scope);
                            if (principal != null) {
                                SetPrincipal(principal, ExtractAccessTokenFromHeader(authHeader));
        ...
        {
            return authHeader.Substring(HeaderBearer.Length, authHeader.Length - HeaderBearer.Length);
        }
        ...
        protected virtual void SetPrincipal(IPrincipal principal, string accessToken) {
        ...
        private string ExtractScopeFromRequest(string requestUri)
        {
            if (requestUri.Contains("profile"))
                return "profile";
            var i = requestUri.LastIndexOf('/');
            return requestUri.Substring(i + 1);
        }
    }
