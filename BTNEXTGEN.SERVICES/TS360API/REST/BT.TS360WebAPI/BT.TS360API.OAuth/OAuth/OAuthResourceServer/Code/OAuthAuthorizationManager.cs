namespace OAuthResourceServer.Code {
	...
	public class OAuthAuthorizationManager : ServiceAuthorizationManager {
		...
		protected override bool CheckAccessCore(OperationContext operationContext) {
			...
			try {
                var action = operationContext.IncomingMessageHeaders.Action ?? operationContext.IncomingMessageHeaders.To.AbsolutePath;
				var i = action.LastIndexOf('/');
                action = action.Substring(i+1);
                var principal = VerifyOAuth2(httpDetails, requestUri, action);
