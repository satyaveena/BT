...
$(document).ready(function () {
	...
	function requestAuthorizationButton_onClick(evt) {
		...
		var authorizeUrl = "http://localhost/TS360OAuthAuthorizationServer/OAuth/Authorize" + utilities.assembleQueryString(args);
...
$(document).ready(function () {
	...
	function serviceCall(operation, accessToken, label) {
		label.text('fetching...');
		$.ajax({
		    url: "http://localhost/TS360OAuthResourceServer" + encodeURI(operation),
			headers: {
