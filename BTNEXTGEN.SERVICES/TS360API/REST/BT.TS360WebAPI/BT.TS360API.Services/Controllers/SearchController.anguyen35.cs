...
namespace BT.TS360API.Services.Controllers
{
    public partial class SearchController : ApiController
    {
        ...
        public async Task<AppServiceResult<ProductAltFormatsResponse>> GetProductAltFormats(ProductAltFormatsRequest request)
        {
            var result = new AppServiceResult<ProductAltFormatsResponse>();
            try
            {
                //
                result.Data = await _searchService.GetProductAltFormats(request);
                result.Status = AppServiceStatus.Success;
            }
            catch (Exception ex)
            {
        ...
        public AppServiceResult<WCFObjectReturnToClient> GetStockPriceCheckForQuickSearch(GetStockPriceCheckForQuickSearchRequest request)
        {
            ...
            try
            {
                var searchService = new BT.TS360API.Services.Services.SearchService();
        ...
        public AppServiceResult<BTListReturn> GetAllInfoBTListsPage(BTListsArgRequest request)
        {
            ...
            try
            {
                ...
                if (request != null && request.arg != null
                    && request.arg.EListCatIds != null && request.arg.EListCatIds.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        var subCategories = ContentManagementController.Current.GetEListSubCategoriesItem(request.arg.EListCatIds);
                        var eList = ContentManagementController.Current.GetEListBySubcategory(subCategories.Select(sub => sub.Id).ToList());
