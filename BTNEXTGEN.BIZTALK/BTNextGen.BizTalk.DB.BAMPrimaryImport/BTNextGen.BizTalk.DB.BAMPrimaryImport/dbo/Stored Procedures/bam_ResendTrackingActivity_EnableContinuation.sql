CREATE PROCEDURE [dbo].[bam_ResendTrackingActivity_EnableContinuation]
      (
    ...
    BEGIN
              ...
              UPDATE [dbo].[bam_ResendTrackingActivity_Continuations]
              SET ParentActivityID = @@rootTrace 
              FROM [dbo].[bam_ResendTrackingActivity_Continuations] WITH (INDEX(NCI_ParentActivityID))
              ...
              SELECT @IsComplete = IsComplete FROM [dbo].[bam_ResendTrackingActivity_Active]
              WHERE ActivityID = @ContinuationToken 

              -- Insert continuation marker if it's not completed
              IF (@IsComplete IS NULL) OR (@IsComplete = 0)
              BEGIN
                  INSERT [dbo].[bam_ResendTrackingActivity_Continuations] (ActivityID, ParentActivityID)
                  VALUES (@ContinuationToken, @@rootTrace)
              END

              -- Update the Relationships for the continuation
              UPDATE [dbo].[bam_ResendTrackingActivity_ActiveRelationships]
              SET ActivityID = @@rootTrace 
              WHERE ActivityID = @ContinuationToken

              -- Move the data from the continuation to the parent row 
              DECLARE @CorrelationId NVARCHAR (255)
              DECLARE @AdapterPrefix NVARCHAR (255)
              DECLARE @ResendCount INT 
              DECLARE @MaxResendCount INT 
              DECLARE @ResendInterval INT 
              DECLARE @MaxRetryCount INT 
              DECLARE @RetryInterval INT 
              DECLARE @MessageContentId NVARCHAR (255)
              DECLARE @ResendTimeout INT 
              DECLARE @RetryTimeout INT 
              ...
              FROM [dbo].[bam_ResendTrackingActivity_Active]
              WHERE ActivityID = @ContinuationToken
              ...
              DELETE FROM [dbo].[bam_ResendTrackingActivity_Active] 
              WHERE ActivityID = @ContinuationToken
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[bam_ResendTrackingActivity_EnableContinuation] TO [bam_ResendTrackingActivity_EventWriter]
    AS [dbo];
