CREATE PROCEDURE [dbo].[bam_InterchangeStatusActivity_EnableContinuation]
      (
    ...
    BEGIN
              ...
              WHERE ActivityName = N'InterchangeStatusActivity'
              ...
              BEGIN
                  ...
                  FROM [dbo].[bam_InterchangeStatusActivity_Continuations] WITH (ROWLOCK, REPEATABLEREAD)
                  WHERE ActivityID = @@rootTrace
              END

              -- If current trace has continuations, set their parents to root trace
              UPDATE [dbo].[bam_InterchangeStatusActivity_Continuations]
              SET ParentActivityID = @@rootTrace 
              FROM [dbo].[bam_InterchangeStatusActivity_Continuations] WITH (INDEX(NCI_ParentActivityID))
              ...
              SELECT @IsComplete = IsComplete FROM [dbo].[bam_InterchangeStatusActivity_Active]
              WHERE ActivityID = @ContinuationToken 

              -- Insert continuation marker if it's not completed
              IF (@IsComplete IS NULL) OR (@IsComplete = 0)
              BEGIN
                  INSERT [dbo].[bam_InterchangeStatusActivity_Continuations] (ActivityID, ParentActivityID)
                  VALUES (@ContinuationToken, @@rootTrace)
              END

              -- Update the Relationships for the continuation
              UPDATE [dbo].[bam_InterchangeStatusActivity_ActiveRelationships]
              ...
              DECLARE @EdiMessageType INT 
              DECLARE @PortID NVARCHAR (40)
              DECLARE @IsInterchangeAckExpected INT 
              DECLARE @IsFunctionalAckExpected INT 
              ...
              SELECT 
                  ...
                  @PortID = [PortID],
                  @IsInterchangeAckExpected = [IsInterchangeAckExpected],
                  @IsFunctionalAckExpected = [IsFunctionalAckExpected],
                  @TimeCreated = [TimeCreated],
                  @RowFlags = [RowFlags],
                  @AckCorrelationId = [AckCorrelationId],
                  @TsCorrelationId = [TsCorrelationId],
                  @AgreementName = [AgreementName]
              FROM [dbo].[bam_InterchangeStatusActivity_Active]
              WHERE ActivityID = @ContinuationToken
              
              EXEC [dbo].[bam_InterchangeStatusActivity_UpsertInstance]
              ...
              DELETE FROM [dbo].[bam_InterchangeStatusActivity_Active] 
              WHERE ActivityID = @ContinuationToken
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[bam_InterchangeStatusActivity_EnableContinuation] TO [bam_InterchangeStatusActivity_EventWriter]
    AS [dbo];
