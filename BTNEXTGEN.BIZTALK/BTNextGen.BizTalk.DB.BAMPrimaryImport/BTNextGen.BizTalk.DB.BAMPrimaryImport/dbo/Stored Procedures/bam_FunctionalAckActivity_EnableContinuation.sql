CREATE PROCEDURE [dbo].[bam_FunctionalAckActivity_EnableContinuation]
      (
    ...
    BEGIN
              ...
              WHERE ActivityName = N'FunctionalAckActivity'
              ...
              BEGIN
                  ...
                  FROM [dbo].[bam_FunctionalAckActivity_Continuations] WITH (ROWLOCK, REPEATABLEREAD)
                  WHERE ActivityID = @@rootTrace
              END

              -- If current trace has continuations, set their parents to root trace
              UPDATE [dbo].[bam_FunctionalAckActivity_Continuations]
              SET ParentActivityID = @@rootTrace 
              FROM [dbo].[bam_FunctionalAckActivity_Continuations] WITH (INDEX(NCI_ParentActivityID))
              ...
              DECLARE @DeliveredTSCount INT 
              DECLARE @AcceptedTSCount INT 
              ...
              DECLARE @ErrorCode4 INT 
              DECLARE @ErrorCode5 INT 
              DECLARE @TimeCreated DATETIME 
              DECLARE @RowFlags INT 
              DECLARE @AgreementName NVARCHAR (256)
              
              SELECT 
                  @InterchangeActivityID = [InterchangeActivityID],
                  ...
                  @DeliveredTSCount = [DeliveredTSCount],
                  @AcceptedTSCount = [AcceptedTSCount],
                  @AckIcn = [AckIcn],
              ...
              FROM [dbo].[bam_FunctionalAckActivity_Active]
              WHERE ActivityID = @ContinuationToken
              
              EXEC [dbo].[bam_FunctionalAckActivity_UpsertInstance]
              ...
              DELETE FROM [dbo].[bam_FunctionalAckActivity_Active] 
              WHERE ActivityID = @ContinuationToken
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[bam_FunctionalAckActivity_EnableContinuation] TO [bam_FunctionalAckActivity_EventWriter]
    AS [dbo];
