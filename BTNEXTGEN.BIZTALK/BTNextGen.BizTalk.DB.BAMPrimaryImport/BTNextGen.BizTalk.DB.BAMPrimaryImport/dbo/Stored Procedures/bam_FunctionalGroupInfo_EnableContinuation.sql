CREATE PROCEDURE [dbo].[bam_FunctionalGroupInfo_EnableContinuation]
      (
    ...
    BEGIN
              ...
              WHERE ActivityName = N'FunctionalGroupInfo'
              ...
              BEGIN
                  ...
                  FROM [dbo].[bam_FunctionalGroupInfo_Continuations] WITH (ROWLOCK, REPEATABLEREAD)
                  WHERE ActivityID = @@rootTrace
              END

              -- If current trace has continuations, set their parents to root trace
              UPDATE [dbo].[bam_FunctionalGroupInfo_Continuations]
              SET ParentActivityID = @@rootTrace 
              FROM [dbo].[bam_FunctionalGroupInfo_Continuations] WITH (INDEX(NCI_ParentActivityID))
              ...
              SELECT @IsComplete = IsComplete FROM [dbo].[bam_FunctionalGroupInfo_Active]
              WHERE ActivityID = @ContinuationToken 

              -- Insert continuation marker if it's not completed
              IF (@IsComplete IS NULL) OR (@IsComplete = 0)
              BEGIN
                  INSERT [dbo].[bam_FunctionalGroupInfo_Continuations] (ActivityID, ParentActivityID)
                  VALUES (@ContinuationToken, @@rootTrace)
              END

              -- Update the Relationships for the continuation
              UPDATE [dbo].[bam_FunctionalGroupInfo_ActiveRelationships]
              ...
              DECLARE @FunctionalIDCode NVARCHAR (6)
              DECLARE @TSCount INT 
              ...
              SELECT 
                  ...
                  @TSCount = [TSCount]
              FROM [dbo].[bam_FunctionalGroupInfo_Active]
              WHERE ActivityID = @ContinuationToken
              
              EXEC [dbo].[bam_FunctionalGroupInfo_UpsertInstance]
              ...
              DELETE FROM [dbo].[bam_FunctionalGroupInfo_Active] 
              WHERE ActivityID = @ContinuationToken
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[bam_FunctionalGroupInfo_EnableContinuation] TO [bam_FunctionalGroupInfo_EventWriter]
    AS [dbo];
