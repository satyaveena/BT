CREATE PROCEDURE [dbo].[bam_ExpiredCreditCards_EnableContinuation]
      (
    ...
    BEGIN
              ...
              WHERE ActivityName = N'ExpiredCreditCards'
              ...
              BEGIN
                  ...
                  FROM [dbo].[bam_ExpiredCreditCards_Continuations] WITH (ROWLOCK, REPEATABLEREAD)
                  WHERE ActivityID = @@rootTrace
              END

              -- If current trace has continuations, set their parents to root trace
              UPDATE [dbo].[bam_ExpiredCreditCards_Continuations]
              SET ParentActivityID = @@rootTrace 
              FROM [dbo].[bam_ExpiredCreditCards_Continuations] WITH (INDEX(NCI_ParentActivityID))
              ...
              SELECT @IsComplete = IsComplete FROM [dbo].[bam_ExpiredCreditCards_Active]
              WHERE ActivityID = @ContinuationToken 

              -- Insert continuation marker if it's not completed
              IF (@IsComplete IS NULL) OR (@IsComplete = 0)
              BEGIN
                  INSERT [dbo].[bam_ExpiredCreditCards_Continuations] (ActivityID, ParentActivityID)
                  VALUES (@ContinuationToken, @@rootTrace)
              END

              -- Update the Relationships for the continuation
              UPDATE [dbo].[bam_ExpiredCreditCards_ActiveRelationships]
              SET ActivityID = @@rootTrace 
              WHERE ActivityID = @ContinuationToken

              -- Move the data from the continuation to the parent row 
              DECLARE @ID_CreditCards NVARCHAR (50)
              DECLARE @ID_UserObjects NVARCHAR (50)
              DECLARE @DateCreated DATETIME 
              DECLARE @Last4Digits NVARCHAR (4)
              DECLARE @ExpirationMonth NVARCHAR (2)
              DECLARE @ExpirationYear NVARCHAR (4)
              DECLARE @Alias NVARCHAR (50)
              DECLARE @EmailAddress NVARCHAR (50)
              
              SELECT 
                  @ID_CreditCards = [ID_CreditCards],
                  @ID_UserObjects = [ID_UserObjects],
              ...
              FROM [dbo].[bam_ExpiredCreditCards_Active]
              WHERE ActivityID = @ContinuationToken
              
              EXEC [dbo].[bam_ExpiredCreditCards_UpsertInstance]
              ...
              DELETE FROM [dbo].[bam_ExpiredCreditCards_Active] 
              WHERE ActivityID = @ContinuationToken
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[bam_ExpiredCreditCards_EnableContinuation] TO [bam_ExpiredCreditCards_EventWriter]
    AS [dbo];
