CREATE PROCEDURE [dbo].[bam_Cybersource Settlement Feed_EnableContinuation]
      (
    ...
    BEGIN
              ...
              WHERE ActivityName = N'Cybersource Settlement Feed'
              ...
              BEGIN
                  ...
                  FROM [dbo].[bam_Cybersource Settlement Feed_Continuations] WITH (ROWLOCK, REPEATABLEREAD)
                  WHERE ActivityID = @@rootTrace
              END

              -- If current trace has continuations, set their parents to root trace
              UPDATE [dbo].[bam_Cybersource Settlement Feed_Continuations]
              SET ParentActivityID = @@rootTrace 
              FROM [dbo].[bam_Cybersource Settlement Feed_Continuations] WITH (INDEX(NCI_ParentActivityID))
              ...
              SELECT @IsComplete = IsComplete FROM [dbo].[bam_Cybersource Settlement Feed_Active]
              WHERE ActivityID = @ContinuationToken 

              -- Insert continuation marker if it's not completed
              IF (@IsComplete IS NULL) OR (@IsComplete = 0)
              BEGIN
                  INSERT [dbo].[bam_Cybersource Settlement Feed_Continuations] (ActivityID, ParentActivityID)
                  VALUES (@ContinuationToken, @@rootTrace)
              END

              -- Update the Relationships for the continuation
              UPDATE [dbo].[bam_Cybersource Settlement Feed_ActiveRelationships]
              SET ActivityID = @@rootTrace 
              WHERE ActivityID = @ContinuationToken

              -- Move the data from the continuation to the parent row 
              DECLARE @TimeStarted DATETIME 
              DECLARE @TimeEnded DATETIME 
              
              SELECT 
                  @TimeStarted = [TimeStarted],
                  @TimeEnded = [TimeEnded]
              FROM [dbo].[bam_Cybersource Settlement Feed_Active]
              WHERE ActivityID = @ContinuationToken
              
              EXEC [dbo].[bam_Cybersource Settlement Feed_UpsertInstance]
              ...
              DELETE FROM [dbo].[bam_Cybersource Settlement Feed_Active] 
              WHERE ActivityID = @ContinuationToken
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[bam_Cybersource Settlement Feed_EnableContinuation] TO [bam_Cybersource Settlement Feed_EventWriter]
    AS [dbo];
