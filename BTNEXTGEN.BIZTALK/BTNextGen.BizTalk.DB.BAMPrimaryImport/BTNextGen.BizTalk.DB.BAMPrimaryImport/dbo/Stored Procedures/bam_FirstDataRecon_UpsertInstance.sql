CREATE PROCEDURE [dbo].[bam_FirstDataRecon_UpsertInstance]
      (
    ...
    BEGIN
            ...
            BEGIN
                ...
                FROM [dbo].[bam_FirstDataRecon_Active] WITH (ROWLOCK, REPEATABLEREAD)
            ...
            BEGIN
                IF (EXISTS (SELECT continuations.ActivityID
                            FROM [dbo].[bam_FirstDataRecon_Continuations] continuations WITH (ROWLOCK, REPEATABLEREAD)
            ...
            BEGIN
                ...
                INSERT [dbo].[bam_FirstDataRecon_Completed]
                (
                    ActivityID,
                    LastModified,
          ...
          [SendTolas]
                ...
                SELECT
                    ...
                    COALESCE(@@ProcessAttachment, primaryImport.[ProcessAttachment]),
                    COALESCE(@@SendTolas, primaryImport.[SendTolas])
                FROM  [dbo].[bam_Metadata_Activities] activities
                LEFT JOIN [dbo].[bam_FirstDataRecon_Active] primaryImport ON ISNULL(primaryImport.ActivityID, @@ActivityID) = @@ActivityID
                WHERE ((@@IsVisible = 1) OR (primaryImport.IsVisible = 1)) AND activities.ActivityName = N'FirstDataRecon'
                ...
                DELETE FROM [dbo].[bam_FirstDataRecon_Active] WHERE ActivityID = @@ActivityID
                ...
                INSERT [dbo].[bam_FirstDataRecon_CompletedRelationships](RecordID, ActivityID, ReferenceType, ReferenceName, ReferenceData, ReferenceDataExtend, LongReferenceData)
                    SELECT @@recordID, ActivityID, ReferenceType, ReferenceName, ReferenceData, ReferenceDataExtend, LongReferenceData
                    FROM [dbo].[bam_FirstDataRecon_ActiveRelationships] WHERE ActivityID = @@ActivityID
          

                -- Remove record from active instances relationship table
                DELETE FROM [dbo].[bam_FirstDataRecon_ActiveRelationships] WHERE ActivityID = @@ActivityID
        ...
        Again:
            UPDATE [dbo].[bam_FirstDataRecon_Active]
            SET
                IsVisible = COALESCE(@@IsVisible, primaryImport.IsVisible),
                IsComplete = COALESCE(@@IsComplete, primaryImport.IsComplete),
                LastModified = GETUTCDATE(),
                
                [ProcessAttachment] = COALESCE(@@ProcessAttachment, primaryImport.[ProcessAttachment]),
                [SendTolas] = COALESCE(@@SendTolas, primaryImport.[SendTolas])    
            FROM [dbo].[bam_FirstDataRecon_Active] primaryImport
            WHERE primaryImport.ActivityID = @@ActivityID

            -- if the record was not found, then insert
            IF (@@ROWCOUNT = 0)
            BEGIN
                INSERT [dbo].[bam_FirstDataRecon_Active]
                (
                    ActivityID,
                    IsVisible,
                    IsComplete,
