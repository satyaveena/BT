CREATE PROCEDURE [dbo].[bam_APIDemo_EnableContinuation]
      (
    ...
    BEGIN
              ...
              BEGIN
                  ...
                  FROM [dbo].[bam_APIDemo_Continuations] WITH (ROWLOCK, REPEATABLEREAD)
                  WHERE ActivityID = @@rootTrace
              END

              -- If current trace has continuations, set their parents to root trace
              UPDATE [dbo].[bam_APIDemo_Continuations]
              SET ParentActivityID = @@rootTrace 
              FROM [dbo].[bam_APIDemo_Continuations] WITH (INDEX(NCI_ParentActivityID))
              ...
              SELECT 
                  ...
                  @Data2 = [Data2],
                  @Data3 = [Data3]
              FROM [dbo].[bam_APIDemo_Active]
              WHERE ActivityID = @ContinuationToken
              
              EXEC [dbo].[bam_APIDemo_UpsertInstance]
              ...
              DELETE FROM [dbo].[bam_APIDemo_Active] 
              WHERE ActivityID = @ContinuationToken
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[bam_APIDemo_EnableContinuation] TO [bam_APIDemo_EventWriter]
    AS [dbo];
