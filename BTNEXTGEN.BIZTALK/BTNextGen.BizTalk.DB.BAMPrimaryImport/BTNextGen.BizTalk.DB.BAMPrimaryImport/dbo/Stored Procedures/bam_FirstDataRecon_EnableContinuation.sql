CREATE PROCEDURE [dbo].[bam_FirstDataRecon_EnableContinuation]
      (
    ...
    BEGIN
              ...
              WHERE ActivityName = N'FirstDataRecon'
              ...
              BEGIN
                  ...
                  FROM [dbo].[bam_FirstDataRecon_Continuations] WITH (ROWLOCK, REPEATABLEREAD)
                  WHERE ActivityID = @@rootTrace
              END

              -- If current trace has continuations, set their parents to root trace
              UPDATE [dbo].[bam_FirstDataRecon_Continuations]
              SET ParentActivityID = @@rootTrace 
              FROM [dbo].[bam_FirstDataRecon_Continuations] WITH (INDEX(NCI_ParentActivityID))
              ...
              SELECT @IsComplete = IsComplete FROM [dbo].[bam_FirstDataRecon_Active]
              WHERE ActivityID = @ContinuationToken 

              -- Insert continuation marker if it's not completed
              IF (@IsComplete IS NULL) OR (@IsComplete = 0)
              BEGIN
                  INSERT [dbo].[bam_FirstDataRecon_Continuations] (ActivityID, ParentActivityID)
                  VALUES (@ContinuationToken, @@rootTrace)
              END

              -- Update the Relationships for the continuation
              UPDATE [dbo].[bam_FirstDataRecon_ActiveRelationships]
              SET ActivityID = @@rootTrace 
              WHERE ActivityID = @ContinuationToken

              -- Move the data from the continuation to the parent row 
              DECLARE @ProcessAttachment DATETIME 
              DECLARE @SendTolas DATETIME 
              
              SELECT 
                  @ProcessAttachment = [ProcessAttachment],
                  @SendTolas = [SendTolas]
              FROM [dbo].[bam_FirstDataRecon_Active]
              WHERE ActivityID = @ContinuationToken
              
              EXEC [dbo].[bam_FirstDataRecon_UpsertInstance]
              ...
              DELETE FROM [dbo].[bam_FirstDataRecon_Active] 
              WHERE ActivityID = @ContinuationToken
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[bam_FirstDataRecon_EnableContinuation] TO [bam_FirstDataRecon_EventWriter]
    AS [dbo];
