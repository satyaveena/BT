CREATE PROCEDURE [dbo].[bam_TransactionSetActivity_EnableContinuation]
      (
    ...
    BEGIN
              ...
              WHERE ActivityName = N'TransactionSetActivity'
              ...
              BEGIN
                  ...
                  FROM [dbo].[bam_TransactionSetActivity_Continuations] WITH (ROWLOCK, REPEATABLEREAD)
                  WHERE ActivityID = @@rootTrace
              END

              -- If current trace has continuations, set their parents to root trace
              UPDATE [dbo].[bam_TransactionSetActivity_Continuations]
              SET ParentActivityID = @@rootTrace 
              FROM [dbo].[bam_TransactionSetActivity_Continuations] WITH (INDEX(NCI_ParentActivityID))
              ...
              SELECT @IsComplete = IsComplete FROM [dbo].[bam_TransactionSetActivity_Active]
              WHERE ActivityID = @ContinuationToken 

              -- Insert continuation marker if it's not completed
              IF (@IsComplete IS NULL) OR (@IsComplete = 0)
              BEGIN
                  INSERT [dbo].[bam_TransactionSetActivity_Continuations] (ActivityID, ParentActivityID)
                  VALUES (@ContinuationToken, @@rootTrace)
              END

              -- Update the Relationships for the continuation
              UPDATE [dbo].[bam_TransactionSetActivity_ActiveRelationships]
              ...
              DECLARE @ApplicationSender NVARCHAR (41)
              DECLARE @ApplicationReceiver NVARCHAR (41)
              DECLARE @GroupDateTime DATETIME 
              DECLARE @GroupControlNo NVARCHAR (14)
              DECLARE @TransactionSetId NVARCHAR (6)
              DECLARE @DocType NVARCHAR (256)
              DECLARE @TransactionSetControlNo NVARCHAR (14)
              DECLARE @AckStatusCode INT 
              DECLARE @BatchProcessing INT 
              DECLARE @ProcessingDateTime DATETIME 
              DECLARE @GroupOrdinal INT 
              DECLARE @TransactionSetOrdinal INT 
              DECLARE @MessageContentKey NVARCHAR (40)
              DECLARE @TimeCreated DATETIME 
              DECLARE @RowFlags INT 
              DECLARE @TsCorrelationId NVARCHAR (32)
              ...
              SELECT 
                  ...
                  @GroupControlNo = [GroupControlNo],
                  @TransactionSetId = [TransactionSetId],
                  @DocType = [DocType],
                  @TransactionSetControlNo = [TransactionSetControlNo],
                  @AckStatusCode = [AckStatusCode],
                  @BatchProcessing = [BatchProcessing],
                  @ProcessingDateTime = [ProcessingDateTime],
                  @GroupOrdinal = [GroupOrdinal],
                  @TransactionSetOrdinal = [TransactionSetOrdinal],
                  @MessageContentKey = [MessageContentKey],
                  @TimeCreated = [TimeCreated],
                  @RowFlags = [RowFlags],
                  @TsCorrelationId = [TsCorrelationId],
                  @AgreementName = [AgreementName]
              FROM [dbo].[bam_TransactionSetActivity_Active]
              WHERE ActivityID = @ContinuationToken
              
              EXEC [dbo].[bam_TransactionSetActivity_UpsertInstance]
              ...
              DELETE FROM [dbo].[bam_TransactionSetActivity_Active] 
              WHERE ActivityID = @ContinuationToken
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[bam_TransactionSetActivity_EnableContinuation] TO [bam_TransactionSetActivity_EventWriter]
    AS [dbo];
