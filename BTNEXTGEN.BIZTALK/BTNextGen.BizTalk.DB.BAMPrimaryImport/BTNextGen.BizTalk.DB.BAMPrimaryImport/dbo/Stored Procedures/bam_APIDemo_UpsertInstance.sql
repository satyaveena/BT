CREATE PROCEDURE [dbo].[bam_APIDemo_UpsertInstance]
      (
    ...
    BEGIN
            ...
            BEGIN
                ...
                FROM [dbo].[bam_APIDemo_Active] WITH (ROWLOCK, REPEATABLEREAD)
            ...
            BEGIN
                IF (EXISTS (SELECT continuations.ActivityID
                            FROM [dbo].[bam_APIDemo_Continuations] continuations WITH (ROWLOCK, REPEATABLEREAD)
            ...
            BEGIN
                ...
                INSERT [dbo].[bam_APIDemo_Completed]
                (
                    ActivityID,
                    LastModified,
          ...
          [Data3]
                ...
                SELECT
                    ...
                    COALESCE(@@Key, primaryImport.[Key]),
                    COALESCE(@@Data1, primaryImport.[Data1]),
                    COALESCE(@@Data2, primaryImport.[Data2]),
                    COALESCE(@@Data3, primaryImport.[Data3])
                FROM  [dbo].[bam_Metadata_Activities] activities
                LEFT JOIN [dbo].[bam_APIDemo_Active] primaryImport ON ISNULL(primaryImport.ActivityID, @@ActivityID) = @@ActivityID
                WHERE ((@@IsVisible = 1) OR (primaryImport.IsVisible = 1)) AND activities.ActivityName = N'APIDemo'
                ...
                DELETE FROM [dbo].[bam_APIDemo_Active] WHERE ActivityID = @@ActivityID
                ...
                INSERT [dbo].[bam_APIDemo_CompletedRelationships](RecordID, ActivityID, ReferenceType, ReferenceName, ReferenceData, ReferenceDataExtend, LongReferenceData)
                    SELECT @@recordID, ActivityID, ReferenceType, ReferenceName, ReferenceData, ReferenceDataExtend, LongReferenceData
                    FROM [dbo].[bam_APIDemo_ActiveRelationships] WHERE ActivityID = @@ActivityID
          

                -- Remove record from active instances relationship table
                DELETE FROM [dbo].[bam_APIDemo_ActiveRelationships] WHERE ActivityID = @@ActivityID
        ...
        Again:
            UPDATE [dbo].[bam_APIDemo_Active]
            SET
                IsVisible = COALESCE(@@IsVisible, primaryImport.IsVisible),
                IsComplete = COALESCE(@@IsComplete, primaryImport.IsComplete),
                LastModified = GETUTCDATE(),
                
                [Key] = COALESCE(@@Key, primaryImport.[Key]),
                [Data1] = COALESCE(@@Data1, primaryImport.[Data1]),
                [Data2] = COALESCE(@@Data2, primaryImport.[Data2]),
                [Data3] = COALESCE(@@Data3, primaryImport.[Data3])    
            FROM [dbo].[bam_APIDemo_Active] primaryImport
            WHERE primaryImport.ActivityID = @@ActivityID

            -- if the record was not found, then insert
            IF (@@ROWCOUNT = 0)
            BEGIN
                INSERT [dbo].[bam_APIDemo_Active]
                (
                    ActivityID,
                    IsVisible,
                    IsComplete,
