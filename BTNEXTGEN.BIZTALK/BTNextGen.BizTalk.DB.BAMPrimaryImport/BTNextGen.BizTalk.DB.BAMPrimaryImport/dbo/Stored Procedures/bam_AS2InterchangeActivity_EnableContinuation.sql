CREATE PROCEDURE [dbo].[bam_AS2InterchangeActivity_EnableContinuation]
      (
    ...
    BEGIN
              ...
              WHERE ActivityName = N'AS2InterchangeActivity'
              ...
              BEGIN
                  ...
                  FROM [dbo].[bam_AS2InterchangeActivity_Continuations] WITH (ROWLOCK, REPEATABLEREAD)
                  WHERE ActivityID = @@rootTrace
              END

              -- If current trace has continuations, set their parents to root trace
              UPDATE [dbo].[bam_AS2InterchangeActivity_Continuations]
              SET ParentActivityID = @@rootTrace 
              FROM [dbo].[bam_AS2InterchangeActivity_Continuations] WITH (INDEX(NCI_ParentActivityID))
              ...
              SELECT @IsComplete = IsComplete FROM [dbo].[bam_AS2InterchangeActivity_Active]
              WHERE ActivityID = @ContinuationToken 

              -- Insert continuation marker if it's not completed
              IF (@IsComplete IS NULL) OR (@IsComplete = 0)
              BEGIN
                  INSERT [dbo].[bam_AS2InterchangeActivity_Continuations] (ActivityID, ParentActivityID)
                  VALUES (@ContinuationToken, @@rootTrace)
              END

              -- Update the Relationships for the continuation
              UPDATE [dbo].[bam_AS2InterchangeActivity_ActiveRelationships]
              ...
              FROM [dbo].[bam_AS2InterchangeActivity_Active]
              WHERE ActivityID = @ContinuationToken
              
              EXEC [dbo].[bam_AS2InterchangeActivity_UpsertInstance]
              ...
              DELETE FROM [dbo].[bam_AS2InterchangeActivity_Active] 
              WHERE ActivityID = @ContinuationToken
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[bam_AS2InterchangeActivity_EnableContinuation] TO [bam_AS2InterchangeActivity_EventWriter]
    AS [dbo];
