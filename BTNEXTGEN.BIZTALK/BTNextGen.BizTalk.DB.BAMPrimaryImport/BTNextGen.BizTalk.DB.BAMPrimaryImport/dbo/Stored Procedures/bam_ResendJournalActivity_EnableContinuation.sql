CREATE PROCEDURE [dbo].[bam_ResendJournalActivity_EnableContinuation]
      (
    ...
    BEGIN
              ...
              WHERE ActivityName = N'ResendJournalActivity'
              ...
              BEGIN
                  ...
                  FROM [dbo].[bam_ResendJournalActivity_Continuations] WITH (ROWLOCK, REPEATABLEREAD)
                  WHERE ActivityID = @@rootTrace
              END

              -- If current trace has continuations, set their parents to root trace
              UPDATE [dbo].[bam_ResendJournalActivity_Continuations]
              SET ParentActivityID = @@rootTrace 
              FROM [dbo].[bam_ResendJournalActivity_Continuations] WITH (INDEX(NCI_ParentActivityID))
              ...
              SELECT @IsComplete = IsComplete FROM [dbo].[bam_ResendJournalActivity_Active]
              WHERE ActivityID = @ContinuationToken 

              -- Insert continuation marker if it's not completed
              IF (@IsComplete IS NULL) OR (@IsComplete = 0)
              BEGIN
                  INSERT [dbo].[bam_ResendJournalActivity_Continuations] (ActivityID, ParentActivityID)
                  VALUES (@ContinuationToken, @@rootTrace)
              END

              -- Update the Relationships for the continuation
              UPDATE [dbo].[bam_ResendJournalActivity_ActiveRelationships]
              SET ActivityID = @@rootTrace 
              WHERE ActivityID = @ContinuationToken

              -- Move the data from the continuation to the parent row 
              DECLARE @TrackingActivityID NVARCHAR (255)
              DECLARE @ResendIndex INT 
              DECLARE @ResendStatus NVARCHAR (50)
              ...
              FROM [dbo].[bam_ResendJournalActivity_Active]
              WHERE ActivityID = @ContinuationToken
              
              EXEC [dbo].[bam_ResendJournalActivity_UpsertInstance]
              ...
              DELETE FROM [dbo].[bam_ResendJournalActivity_Active] 
              WHERE ActivityID = @ContinuationToken
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[bam_ResendJournalActivity_EnableContinuation] TO [bam_ResendJournalActivity_EventWriter]
    AS [dbo];
