CREATE FUNCTION bts_AllDuplicateX12InterchangeStatusRecordIDs()
RETURNS TABLE 
AS
	Return
	(
		...
		(0 < ( select count(1) from bam_InterchangeStatusActivity_CompletedInstances inTable 
			...
			mainTable.Direction = inTable.Direction AND mainTable.EdiMessageType=0
			AND (inTable.RecordID > mainTable.RecordID)
		)) 
	)
GO
GRANT SELECT
    ON OBJECT::[dbo].[bts_AllDuplicateX12InterchangeStatusRecordIDs] TO [BTS_ADMIN_USERS]
    AS [dbo];


GO
GRANT SELECT
    ON OBJECT::[dbo].[bts_AllDuplicateX12InterchangeStatusRecordIDs] TO [BTS_OPERATORS]
    AS [dbo];
